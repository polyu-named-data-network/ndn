...
func (interestHandler_s) HandleError(err error) {
  utils.Error("failed to listen on incoming interest socket", err)
}

/* REMARK: this function is blocking */
func (p interestHandler_s) HandleConnection(conn net.Conn) {
  p.wg.Add(1)
  utils.Info("client connected to interest service", conn.RemoteAddr().Network(), conn.RemoteAddr().String())
  ...
  go func() {
    ...
    for err == nil {
      utils.Info("wait for incoming interest packet")
      err = decoder.Decode(&in_packet)
      if err != nil {
        if err != io.EOF {
          utils.Error("failed to decode incoming interest packet", err)
        }
      } else {
        utils.Info("received interest packet", in_packet)
